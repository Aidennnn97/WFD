...
public class LaligaController {
	...
	@RequestMapping("/laliga")
	   public String laligaJsp(Model model) {
	   ...
	   //   오늘 경기 일정 크롤링 데이터 
	   @RequestMapping("/craw/laligaPlan.ajax")
	   @ResponseBody
	   public ArrayList<HashMap<String, String>> laligaTodayMatch()throws Exception {

	      // DataStorage VO의 변수에 담긴 프리미어리그 경기일정 크롤링 데이터 리턴.
	      
	      return LaligaDataStorage.getTodayLaligaMatchData();
	   ...
	   @RequestMapping("/craw/clickLaligaMatchData.ajax")
	   @ResponseBody
	   public ArrayList<HashMap<String, String>> laligaMatchDetail(String gameId)throws Exception {
	      // 경기 상세 정보 크롤링 모듈.
	   ...
	   // 전체 팀 순위 크롤링 데이터 
	   @RequestMapping("/craw/laligaTeamRank.ajax")
	   @ResponseBody
	   public ArrayList<HashMap<String, String>> laligaTeamRank()throws Exception {

	      return LaligaDataStorage.getLaligaTeamRankData();

	   }


	   // 전체 선수 순위 크롤링데이터 
	   @RequestMapping("/craw/laligaPlayerRank.ajax")
	   @ResponseBody
	   public ArrayList<HashMap<String, String>> laligaPlayerRank()throws Exception {

	      return LaligaDataStorage.getLaligaPlayerRankData();
	   ...
	   @RequestMapping("/craw/laligaPlayerAsistRank.ajax")
	   @ResponseBody
	   public ArrayList<HashMap<String, String>> laligaPlayerAsistRank()throws Exception {
	      
	      return LaligaDataStorage.getLaligaPlayerAsistRankData();
	      
	   }
	   

	   //팀 내 모든선수 크롤링데이터 
	   @RequestMapping("/craw/innerLaligaPlayerRank.ajax")
	   @ResponseBody
	   public ArrayList<HashMap<String, String>> innerLaligaPlayerRank(String teamId)throws Exception{
	      // 모든 팀 내의 선수단 정보를 받아올 그릇 (리그 내 전체 선수 데이터)
	            ...
	            try {
	               ...
	               // 팀 상세페이지의 데이터 크롤링 주소 
	               String url = "https://sports.daum.net/prx/hermes/api/person/list.json?leagueCode=primera&seasonKey=20222023&teamId="+ teamId +"&detail=true&pageSize=100";
