<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
		
				<!-- Home Pomation Start -->
				"<div class='match'>"+
					"<div class='goalkeeperBox' style='width:20%;'>"+
						"<div class='player_info'>"+
							"<div class='player_imgBox' style=' height: 40px;'>"+
								"<img class='player_img'class='player_img' src='"+homeArray[0].homePlayerImg+"'>"+
							"</div>"+
							"<div class='player_personal_info'>"+
								"<div class='player_no'>"+homeArray[0].homePlayerBackNumber+"</div>"+
								"."+
								"<div class='player_name'>"+homeArray[0].homePlayerName+"</div>"+
							"</div>"+
						"</div>"+
					"</div>"+
					"<div class='defendersBox' style='width:20%; justify-content: space-around;'>"+
						"<div class='player_info'>"+
							"<div class='player_imgBox'style='height: 40px;'>"+
								"<img class='player_img'class='player_img' src='"+homeArray[1].homePlayerImg+"'>"+
							"</div>"+
							"<div class='player_personal_info'>"+
								"<div class='player_no'>"+homeArray[1].homePlayerBackNumber+"</div>"+
								"."+
								"<div class='player_name'>"+homeArray[1].homePlayerName+"</div>"+
							"</div>"+
						"</div>"+
						"<div class='player_info'>"+
							"<div class='player_imgBox'style='height: 40px;'>"+
								"<img class='player_img'class='player_img' src='"+homeArray[2].homePlayerImg+"'>"+
							"</div>"+
							"<div class='player_personal_info'>"+
								"<div class='player_no'>"+homeArray[2].homePlayerBackNumber+"</div>"+
								"."+
								"<div class='player_name'>"+homeArray[2].homePlayerName+"</div>"+
							"</div>"+
						"</div>"+
						"<div class='player_info'>"+
							"<div class='player_imgBox'style='height: 40px;'>"+
								"<img class='player_img'class='player_img' src='"+homeArray[3].homePlayerImg+"'>"+
							"</div>"+
							"<div class='player_personal_info'>"+
								"<div class='player_no'>"+homeArray[3].homePlayerBackNumber+"</div>"+
								"."+
								"<div class='player_name'>"+homeArray[3].homePlayerName+"</div>"+
							"</div>"+
						"</div>"+
					"</div>"+
					"<div class='midfieldersBox' style='width:20%; justify-content: space-evenly;'>"+
						"<div class='player_info'>"+
							"<div class='player_imgBox'style='height: 40px;'>"+
								"<img class='player_img'class='player_img' src='"+homeArray[4].homePlayerImg+"'>"+
							"</div>"+
							"<div class='player_personal_info'>"+
								"<div class='player_no'>"+homeArray[4].homePlayerBackNumber+"</div>"+
								"."+
								"<div class='player_name'>"+homeArray[4].homePlayerName+"</div>"+
							"</div>"+
						"</div>"+
						"<div class='player_info'>"+
							"<div class='player_imgBox'style='height: 40px;'>"+
								"<img class='player_img'class='player_img' src='"+homeArray[5].homePlayerImg+"'>"+
							"</div>"+
							"<div class='player_personal_info'>"+
								"<div class='player_no'>"+homeArray[5].homePlayerBackNumber+"</div>"+
								"."+
								"<div class='player_name'>"+homeArray[5].homePlayerName+"</div>"+
							"</div>"+
						"</div>"+
						"<div class='player_info'>"+
							"<div class='player_imgBox'style='height: 40px;'>"+
								"<img class='player_img'class='player_img' src='"+homeArray[6].homePlayerImg+"'>"+
							"</div>"+
							"<div class='player_personal_info'>"+
								"<div class='player_no'>"+homeArray[6].homePlayerBackNumber+"</div>"+
								"."+
								"<div class='player_name'>"+homeArray[6].homePlayerName+"</div>"+
							"</div>"+
						"</div>"+
						"<div class='player_info'>"+
							"<div class='player_imgBox'style='height: 40px;'>"+
								"<img class='player_img'class='player_img' src='"+homeArray[7].homePlayerImg+"'>"+
							"</div>"+
							"<div class='player_personal_info'>"+
								"<div class='player_no'>"+homeArray[7].homePlayerBackNumber+"</div>"+
								"."+
								"<div class='player_name'>"+homeArray[7].homePlayerName+"</div>"+
							"</div>"+
						"</div>"+
						"<div class='player_info'>"+
							"<div class='player_imgBox'style='height: 40px;'>"+
								"<img class='player_img'class='player_img' src='"+homeArray[8].homePlayerImg+"'>"+
							"</div>"+
							"<div class='player_personal_info'>"+
								"<div class='player_no'>"+homeArray[8].homePlayerBackNumber+"</div>"+
								"."+
								"<div class='player_name'>"+homeArray[8].homePlayerName+"</div>"+
							"</div>"+
						"</div>"+
					"</div>"+
					"<div class='forwardsBox' style='width:20%; justify-content: space-evenly;'>"+
						"<div class='player_info'>"+
							"<div class='player_imgBox'style='height: 40px;'>"+
								"<img class='player_img'class='player_img' src='"+homeArray[9].homePlayerImg+"'>"+
							"</div>"+
							"<div class='player_personal_info'>"+
								"<div class='player_no'>"+homeArray[9].homePlayerBackNumber+"</div>"+
								"."+
								"<div class='player_name'>"+homeArray[9].homePlayerName+"</div>"+
							"</div>"+
						"</div>"+
					"</div>"+
					
					"<div class='strikerBox' style='width:20%;'>"+
						"<div class='player_info'>"+
							"<div class='player_imgBox'style='height: 40px;'>"+
								"<img class='player_img'class='player_img' src='"+homeArray[10].homePlayerImg+"'>"+
							"</div>"+
							"<div class='player_personal_info'>"+
								"<div class='player_no'>"+homeArray[10].homePlayerBackNumber+"</div>"+
								"."+
								"<div class='player_name'>"+homeArray[10].homePlayerName+"</div>"+
							"</div>"+
						"</div>"+
					"</div>"+
					

				"</div>"
				<!-- Home Pomation Finish -->